...
Проект с приложением (в перспективе - веб-сайтом), который использует эту библиотеку.
...
Всё, что свазано с докером. В основном, это описания образов.
...
**mylib/build-tools, myapp/build-tools:**
...
Таким образом я решил оставить в корне tsconfig, и next.config.js.
Остальное уехало в build-tools.

**mylib/out:** Туда я компилирую TS -> JS, Сторибуки, Code Coverage, и кэш от awesome-typescript-loader.

**mylib/src, myapp/src:** Исходный код.

**mylib/stories, myapp/stories:** Страницы сторибука.

**mylib/test, myapp/test:** Юнит тесты и запускалка сторишотов.
...
**Тесты гоняем поверх скомпилированного кода.** Типа, что публикуем, то и тестируем.
Поэтому Jest в библиотеке не использует TS пресет.

**Сторибук для сторишотов тоже собираем из скомпилированных компонентов.**
Эта конфигурация тоже не использует TS пресетов.
...
есть вторая конфигурация, уже с поддержкой TS.

**В скомпилировнный код мы кидаем JSX как есть, без трансформации.**
Это потоу что React - не единственная библиотека, которая может работать с JSX.
Мы даём возможность пользователю библиотеки потом решит, что делать с JSX компонентами.

**CSS тоже никак не преобразуем.**
Допускаю мысль, что лучше было бы их компилировать в JS компонент,
но пока кажется, что пользователь библиотеки прекрасно справляется с компиляцией
стилевых файлов без нашей помощи.

**Компилируем в target ES2015 и модули CommonJS** для того,
...
Приложение на данный момент - это простейший JS файл,
который импортит тот библиотечный файл,
который импортит другой библиотечный файл по абсолютному пути.
...
Для своего случая считаю избыточным плодить кучу библиотек и заботиться об их струкруре.
А строить заборы из выхода на корневой уровень `../../..` - бесячее занятие.
...
Настроить правильным образом Alpine и pupeeter может быть и не самая сложная в жизни задача, но всё равно требует какого-то времени.

Нет лишнего времени? Пользуйтесь моим образом.
...
Рядом скаждым `style.m.css` автоматически создаётся его декларация: `style.m.css.d.ts`.
...
Для демонстрации, я добавил сюда в сторибук стили от Bootstrap.


#### Кастомные class-names
По умолчанию Modules CSS генерит hash-образные имена классов.
Вероятно, это не имеет значения для внутреннего продукта,
но абсолютно неприемлемо для библиотеки.

Если пользователь библиотеки HTML компонентов хочет переопределить
внешний вид не через продуманное автором API, а путём CSS хаков,
то, я щитаю, мы обязаны предоставить ему такую возможность.
Поэтому никаких рандомайзеров и хэшей в именах классов.

Напрямую к пресету библиотеки этот раздел не относится,
потому что компоненты собираются в конечном продуекте вебпаком,
и его настройки определяются пользователем.

Но тем не менее, здесь такой приём кастомизации class-names используется
при построении Storybook, хотя и настроен исключительно по моему вкусу.

Опишем, что происходит:

* если в настройках не писать ничего, то имя класса будет таким: `_2J_gABPOlXaDf0vgkwhqEG`
* если добавить `localIdentHashPrefix: 'evoja-tstest-lib-hash'`, то получится почти так-же: `_2xnzSj7X4bPHB7GoV3yP32`, просто он хэш будет считать ещё и от нашей строчки, добавленной спереди к ресурсу, это уменьшает вероятность коллизий между разными библиотеками.
* если поставить `localIdentName: 'evoja-tstest-lib-[path][name]__[local]'`, то получится `evoja-tstest-src-lib-blocks-AddButton-m__red`. Тут лишнее `-src` между моим префиксом и остальной частью пути. Чтобы это исправить, идём к следующему шагу:
* добавляем `localIdentContext: path.resolve(__dirname, '../../src')`, получается так: `evoja-tstest-blocks-AddButton-m__red`. Это почти то, что мне нужно, но эта `-m` в конце не нравится, она осталась от расширения файла модуля CSS `*.m.css`.
* Ах, ещё, если бы мой компонент лежал в подпапке: 'blocks/AddButton/AddButton.m.css`, а я это делаю для развесистых компонентов, которые состоят из нескольких файлов, то в имени класса имя файла бы продублировалось именем папки, чего хотелось бы избежать.

Итак, намерение - семантически класс отражает имя компонента, хоть и полное, но всё-таки не путь до его файла. Поэтому приходится писать кастомную `getLocalIdent()` функцию.
Получается симпатичный класс: `evoja-tstest-blocks-AddButton__red`.
(Впрочем, плохо себе представляю, что это будет, когда класс подключен из node_modules. Надо будет доделать функцию `getLocalIdent`).

В общем, обратите внимание - может пригодиться.
...
Тут в целом, ничего особенного, но, тоже хорошо иметь пример под рукой, чем не иметь.
Что зарешали:
...
* выверенный порядок шагов в джобе.
...
Если вы написали универсальные JSX вьюхи, то они могу быть использованы с любым движком. Например Preact. В этой ситуации непонятно, зачем ограничивать выбор движка декларацией в _peerDependencies_.

С другой стороны, если ваши компоненты заточены на хитрый реактовский API, то разумно декларировать и то, что это должен быть именно Реакт, и его минимальную версию.
...
Я склоняюсь к тому, что для библиотек, лучше выбирать первый подход, а для фреймворков - второй. Однако у меня нет ответа на вопрос, как определить является ли пакет библиотекой или фреймворком.


#### scripts.mypublish
Исходники у меня лежат в `src`, а компилирую я всё в `out/compiled/src`. Однако `npm publish` работает от корня проекта. Это не имеет значения, когда в вашем пакете есть единая точка входа, которая указана полем **main** в вашем `package.json`. Но у меня нет одной точки. Каждый файл импортируется пользователем по мере необходимости независимо других файлов библиотеки.
Поэтому с одной стороны хочется избавиться от лишнего уровня импортах:

    import from 'mylibrary/out/compiled/src/tool' // это "out" в пути кажется лишним
    import from 'mylibrary/lib/tool' // лучше но всё равно можно было бы и без "lib" обойтись

хочется без лишнего фрагмента:

    import from 'mylibary/tool'`

Но в то же время не хотелось бы билдить в корень. Поэтому мы притворяемся что корень проекта у нас в `out/compiled/src` и публикуем оттуда.
...
Мы хотим компилировать в другую папку (out), однако `tsc` не трогает ничего кроме собственно тайп-скриптовых файлов. Поэтому всё остальное нужно перенести в out самостоятельно: и стили, и картинки, и JS файлы.
...
**myinstall:** мой `package.json` выразителен, там есть отступы пустыми строчками, порядок пакетов зависимостей для важен. NPM норовит испортить мой прекрасный файлик. Поэтому я пользуюсь командой `npm run myinstall` вместо `npm install`.
...
По умолчанию юнит-тесты бегут в джествоском окружении `jsdom` (это настраивается в конфиге), там доступен mock для `window` и прочих браузерных плюшек.

Но для каждого из файлов можно выбрать своё окружение, так, для демонстрации, в файле `add-util.test.ts` я поставил окружение `node`. Там ничего браузерного нет, а наоборот, есть всякое нодовское.
